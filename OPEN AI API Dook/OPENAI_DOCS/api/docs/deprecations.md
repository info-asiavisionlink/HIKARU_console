...
fetched_at: 2026-07-27T01:55:25Z
---
...
To migrate, move reusable prompt content into your application code. See [Migrate from prompt objects](guides/prompting/migrate-from-prompt-object.md).
...
See [Moving from OpenAI Evals to Promptfoo](/cookbook/examples/evaluation/moving-from-openai-evals-to-promptfoo) for a migration path.
...
See [Migrate from Agent Builder](guides/agent-builder/migrate-from-agent-builder.md) to continue with the Agents SDK or ChatGPT Workspace Agents.
...
There are a few key differences between the interfaces in the Realtime beta API and the released GA API. See [the migration guide](guides/realtime.md) for the current GA interface and related Realtime docs.
...
When we released the [Responses API](api-reference/responses/create.md) in [March 2025](changelog.md), we announced plans to bring all Assistants API features to the easier to use Responses API, with a sunset date in 2026.

See the Assistants to Conversations [migration guide](assistants/migration.md) to learn more about how to migrate your current integration to the Responses API and Conversations API.
...
In [April 2024](assistants/whats-new.md) when we released the v2 beta version of the Assistants API, we announced that access to the v1 beta would be shut off by the end of 2024. Access to the v1 beta will be discontinued on December 18, 2024.

See the Assistants API v2 beta [migration guide](assistants/migration.md) to learn more about how to migrate your tool usage to the latest version of the Assistants API.
