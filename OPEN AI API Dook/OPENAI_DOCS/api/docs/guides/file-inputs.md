---
source_url: https://developers.openai.com/api/docs/guides/file-inputs
fetched_at: 2026-07-27T01:55:19Z
---
...
- Use [File Search](tools-file-search.md) for retrieval over large files instead of passing them directly as `input_file`.
- Use [Hosted Shell](tools-shell.md) for spreadsheet-heavy tasks that need detailed analysis, such as aggregations, joins, charting, or custom calculations.
...
Chat Completions does not support file URLs. Use the [Responses API](file-inputs.md) for this option.

## Uploading files

The following example uploads a file with the [Files API](../api-reference/files.md), then references its file ID in a request to the model.
...
- **File upload purpose:** You can upload files with any supported [purpose](../api-reference/files/create.md), but use `user_data` for files you plan to pass as model inputs.
