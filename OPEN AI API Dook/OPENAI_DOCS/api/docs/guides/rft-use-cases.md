---
source_url: https://developers.openai.com/api/docs/guides/rft-use-cases
fetched_at: 2026-07-27T01:55:53Z
---
...
[Reinforcement fine-tuning](reinforcement-fine-tuning.md) (RFT) provides a way to improve your model’s performance at specific tasks. The task must be clear and have verifiable answers.
...
[Ready to use reinforcement fine-tuning? Skip to the guide →](reinforcement-fine-tuning.md)
...
> ![Provide example data and create a fine-tuning job to optimize model performance for your use case](../../../assets/API_docs_images_thomsonreuters-results.png)
>
...
An effective eval reveals opportunities where human experts consistently agree but current frontier models struggle, presenting a valuable gap for RFT to close. [Get started with evals](evals.md).
...
- **Make sure each answer can be graded**. A grader must read an answer and produce a score without a person in the loop. We support multiple [grader types](graders.md), including custom Python graders and LLM judges. If you can’t write code to judge the answer with an available grader, RFT is not the right tool.
...
- **Use an LLM judge when code falls short**. For rich, open‑ended answers, have a [separate OpenAI model grade](graders.md) your fine-tuned model’s answers. Make sure you:
...
Learn more about [grader types](graders.md).
...
- [Reinforcement fine-tuning guide](reinforcement-fine-tuning.md)
- [Graders](graders.md)
- [Model optimization overview](model-optimization.md)