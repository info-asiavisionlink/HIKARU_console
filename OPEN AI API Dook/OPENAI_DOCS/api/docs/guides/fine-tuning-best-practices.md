---
source_url: https://developers.openai.com/api/docs/guides/fine-tuning-best-practices
fetched_at: 2026-07-27T01:55:54Z
---
...
After collecting your examples, split the dataset into training and test portions. The training set is for fine-tuning jobs, and the test set is for [evals](evals.md).

When you submit a fine-tuning job with both training and test files, we’ll provide statistics on both during the course of training. These statistics give you signal on how much the model’s improving. Constructing a test set early on helps you [evaluate the model after training](evals.md) by comparing with the test set benchmark.
...
To train the model on [multi-turn conversations](conversation-state.md), include multiple `user` and `assistant` messages in the `messages` array for each line of your training data.
...
Compute token counts with [the tokenizer tool](https://platform.openai.com/tokenizer) or by using code, as in this [cookbook example](/cookbook/examples/how_to_count_tokens_with_tiktoken).
