...
fetched_at: 2026-07-27T01:57:56Z
---
...
- Start with the [MCP and Connectors guide](tools-connectors-mcp.md) for general MCP concepts.
...
![Diagram showing an OpenAI product sending MCP JSON-RPC through the OpenAI tunnel service to tunnel-client, which forwards the request to a private MCP server and returns the response through the same tunnel.](../../../assets/images_platform_guides_secure-mcp-tunnels_request-flow-diagram.png) ![Diagram showing an OpenAI product sending MCP JSON-RPC through the OpenAI tunnel service to tunnel-client, which forwards the request to a private MCP server and returns the response through the same tunnel.](../../../assets/images_platform_guides_secure-mcp-tunnels_request-flow-diagram.png) 
...
[Platform tunnel permissions](rbac.md) and ChatGPT developer-mode access are separate:
...
![Live local tunnel-client admin UI showing health, readiness, tunnel metadata, and channel status.](../../../assets/images_platform_guides_secure-mcp-tunnels_tunnel-client-admin-ui.png) ![Live local tunnel-client admin UI showing health, readiness, tunnel metadata, and channel status.](../../../assets/images_platform_guides_secure-mcp-tunnels_tunnel-client-admin-ui.png) 
...
![Diagram showing tunnel-client inside the customer-controlled environment connecting outbound to the OpenAI-managed tunnel control plane while the private MCP server remains inside the customer network.](../../../assets/images_platform_guides_secure-mcp-tunnels_trust-boundaries-diagram.png) ![Diagram showing tunnel-client inside the customer-controlled environment connecting outbound to the OpenAI-managed tunnel control plane while the private MCP server remains inside the customer network.](../../../assets/images_platform_guides_secure-mcp-tunnels_trust-boundaries-diagram.png) 
...
- Tunnel metadata changes are exposed through the API Platform [Audit logs](/api/reference/resources/admin/subresources/organization/subresources/audit_logs) surface as `tunnel.created`, `tunnel.updated`, and `tunnel.deleted`.
...
[![Sanitized OpenAI Platform tunnel settings screenshot.](../../../assets/images_platform_guides_secure-mcp-tunnels_platform-tunnels-settings.png)](https://platform.openai.com/settings/organization/tunnels)

Create and manage OpenAI-hosted MCP tunnel endpoints from Platform tunnel
settings.

[![Sanitized ChatGPT app creation screenshot with Tunnel selected.](../../../assets/images_platform_guides_secure-mcp-tunnels_chatgpt-connectors-tunnel.png)](https://chatgpt.com/plugins)
