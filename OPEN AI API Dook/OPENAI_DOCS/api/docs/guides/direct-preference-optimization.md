---
source_url: https://developers.openai.com/api/docs/guides/direct-preference-optimization
fetched_at: 2026-07-27T01:55:49Z
---
...
The data should be formatted in JSONL format, with each line [representing an example](../api-reference/fine-tuning/preference-input.md) in the following structure:
...
Uploading training data and using a model fine-tuned with DPO follows the [same flow described here](model-optimization.md).

To create a DPO fine-tune job, use the `method` field in the [fine-tuning job creation endpoint](../api-reference/fine-tuning/create.md), where you can specify `type` as well as any associated `hyperparameters`. For DPO:
