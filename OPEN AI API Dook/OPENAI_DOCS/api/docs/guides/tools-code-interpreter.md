...
fetched_at: 2026-07-27T01:58:05Z
---
...
- Boosting visual intelligence in our latest reasoning models (like [o3](../models/o3.md) and [o4-mini](../models/o4-mini.md)). The model can use this tool to crop, zoom, rotate, and otherwise process and transform images.

Here’s an example of calling the [Responses API](../api-reference/responses.md) with a tool call to Code Interpreter:
...
The Code Interpreter tool requires a [container object](../api-reference/containers/object.md). A container is a fully sandboxed virtual machine that the model can run Python code in. This container can contain files that you upload, or that it generates.
...
2. Explicit mode: here, you explicitly [create a container](../api-reference/containers/createContainers.md) using the `v1/containers` endpoint, including the `memory_limit` you need (for example `"memory_limit": "4g"`), and assign its `id` as the `container` value in the tool configuration in the Response object. For example:
...
You can choose from `1g` (default), `4g`, `16g`, or `64g`. Higher tiers offer more RAM for the session and are billed at the [built-in tools rates](../pricing.md) for Code Interpreter. The selected `memory_limit` applies for the entire life of that container, whether it was created automatically or via the containers API.

Note that containers created with the auto mode are also accessible using the [`/v1/containers`](../api-reference/containers.md) endpoint.
...
You can download these constructed files by calling the [get container file content](../api-reference/container-files/retrieveContainerFileContent.md) method.

Any [files in the model input](file-inputs.md) get automatically uploaded to the container. You do not have to explicitly upload it to the container.

### Uploading and downloading files

Add new files to your container using [Create container file](../api-reference/container-files/createContainerFile.md). This endpoint accepts either a multipart upload or a JSON body with a `file_id`.
List existing container files with [List container files](../api-reference/container-files/listContainerFiles.md) and download bytes from [Retrieve container file content](../api-reference/container-files/retrieveContainerFileContent.md).
...
| --- | --- | --- |
| [Responses](../api-reference/responses.md)  [Chat Completions](../api-reference/chat.md)  [Assistants](../api-reference/assistants.md) | 100 RPM per org | [Pricing](../pricing.md)   [ZDR and data residency](your-data.md) |