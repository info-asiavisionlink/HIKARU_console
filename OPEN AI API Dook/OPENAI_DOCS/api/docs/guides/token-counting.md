---
source_url: https://developers.openai.com/api/docs/guides/token-counting
fetched_at: 2026-07-27T01:55:21Z
---
...
You can use `file_id` (from the [Files API](../api-reference/files.md)) or `image_url` (a URL or base64 data URL). See [images and vision](images-vision.md) for details.
...
[File inputs](pdf-files.md)—currently PDFs—are supported. Pass `file_id`, `file_url`, or `file_data` as you would for `responses.create`. The token count reflects the model’s full processed input.
