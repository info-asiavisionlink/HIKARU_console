---
source_url: https://developers.openai.com/api/docs/guides/reinforcement-fine-tuning
fetched_at: 2026-07-27T01:55:50Z
---
...
Reinforcement fine-tuning (RFT) adapts an OpenAI reasoning model with a feedback signal you define. Like [supervised fine-tuning](supervised-fine-tuning.md), it tailors the model to your task. The difference is that instead of training on fixed “correct” answers, it relies on a programmable grader that scores every candidate response. The training algorithm then shifts the model’s weights, so high-scoring outputs become more likely and low-scoring ones fade.
...
models are [deprecated](../deprecations.md). The full timeline is
[here](../deprecations.md).
...
This optimization lets you align the model with nuanced objectives like style, safety, or domain accuracy—with many [practical use cases](rft-use-cases.md) emerging. Run RFT in five steps:

1. Implement a [grader](graders.md) that assigns a numeric reward to each model response.
2. Upload your prompt dataset and designate a validation split.
3. Start the fine-tune job.
4. Monitor and [evaluate](evals.md) checkpoints; revise data or grader if needed.
...
- **Your task must be compatible with the grading options**. Review [grading options in the API](../api-reference/graders.md) first and verify it’s possible to grade your task with them.
- **Your eval results must be variable enough to improve**. Run [evals](evals.md) before using RFT. If your eval scores between minimum and maximum possible scores, you’ll have enough data to work with to reinforce positive answers. If the model you want to fine-tune scores at either the absolute minimum or absolute maximum score, RFT won’t be useful to you.
...
See common use cases, specific implementations, and grader examples in the [reinforcement fine-tuning use case guide](rft-use-cases.md).
...
currently only for [o4-mini](../models/o4-mini.md).

## Example: LLM-powered security review

To demonstrate reinforcement fine-tuning below, we’ll fine-tune an [o4-mini](../models/o4-mini.md) model to provide expert answers about a fictional company’s security posture, based on an internal company policy document. We want the model to return a JSON object that conforms to a specific schema with [Structured Outputs](structured-outputs.md).
...
To perform RFT, define a [grader](graders.md) to score the model’s output during training, indicating the quality of its response. RFT uses the same set of graders as [evals](evals.md), which you may already be familiar with.

In this example, we define [multiple graders](../api-reference/graders/multi.md) to examine the properties of the JSON returned by our fine-tuned model:

- The [`string_check`](../api-reference/graders/string-check.md) grader to ensure the proper `compliant` property has been set
- The [`score_model`](../api-reference/graders/score-model.md) grader to provide a score between zero and one for the explanation text, using another evaluator model
...
The process for uploading RFT training and test data files is the same as [supervised fine-tuning](supervised-fine-tuning.md). Upload your training data to OpenAI either through the [API](../api-reference/files/create.md) or [using our UI](https://platform.openai.com/storage). Files must be uploaded with a purpose of `fine-tune` in order to be used with fine-tuning.

**You need file IDs for both your test and training data files** to create a fine-tune job.

## Create a fine-tune job

Create a fine-tune job using either the [API](../api-reference/fine-tuning.md) or [fine-tuning dashboard](https://platform.openai.com/finetune). To do this, you need:
...
- To qualify for [data sharing inference pricing](../pricing.md), you need to first [share evaluation and fine-tuning data](https://help.openai.com/en/articles/10306912-sharing-feedback-evaluation-and-fine-tuning-data-and-api-inputs-and-outputs-with-openai#h_c93188c569) with OpenAI before creating the job

### Structured Outputs JSON schema

If you’re fine-tuning a model to return [Structured Outputs](structured-outputs.md), provide the JSON schema being used to format the output. See a valid JSON schema for our security interview use case:
...
This request returns a [fine-tuning job object](../api-reference/fine-tuning/object.md), which includes a job `id`. Use this ID to monitor the progress of your job and retrieve the fine-tuned model when the job is complete.

To qualify for [data sharing inference pricing](../pricing.md), make sure to [share evaluation and fine-tuning data](https://help.openai.com/en/articles/10306912-sharing-feedback-evaluation-and-fine-tuning-data-and-api-inputs-and-outputs-with-openai#h_c93188c569) with OpenAI before creating the job. You can verify the job was marked as shared by confirming `shared_with_openai` is set to `true`.

### Monitoring your fine-tune job

Fine-tuning jobs take some time to complete, and RFT jobs tend to take longer than SFT or DPO jobs. To monitor the progress of your fine-tune job, use the [fine-tuning dashboard](https://platform.openai.com/finetune) or the [API](../api-reference/fine-tuning.md).
...
![Reward Metric Graph](../../../assets/API_images_guides_RFT_Reward_Chart.png)
...
Reinforcement fine-tuning jobs are integrated with our [evals product](evals.md). When you make a reinforcement fine-tuning job, a new eval is automatically created and associated with the job. As validation steps are performed, we combine the input prompts, model samples, and grader outputs to make a new [eval run](evals.md) for that step.
...
Evaluate your newly optimized model by using it! When the fine-tuned model finishes training, use its ID in either the [Responses](../api-reference/responses.md) or [Chat Completions](../api-reference/chat.md) API, just as you would an OpenAI base model.
...
1. Wait until a job succeeds, which you can verify by [querying the status of a job](../api-reference/fine-tuning/retrieve.md).
2. [Query the checkpoints endpoint](../api-reference/fine-tuning/list-checkpoints.md) with your fine-tuning job ID to access a list of model checkpoints for the fine-tuning job.
...
Fine-tune a model by providing correct outputs for sample inputs.](supervised-fine-tuning.md)
[Vision fine-tuning

Learn to fine-tune for computer vision with image inputs.](vision-fine-tuning.md)
[Direct preference optimization

Fine-tune a model using direct preference optimization (DPO).](direct-preference-optimization.md)

## Appendix

### Training metrics

Reinforcement fine-tuning jobs publish per-step training metrics as [fine-tuning events](../api-reference/fine-tuning/event-object.md). Pull these metrics through the [API](../api-reference/fine-tuning/list-events.md) or view them as graphs and charts in the [fine-tuning dashboard](https://platform.openai.com/finetune).
...
![Per-Grader Reward Metric Graph](../../../assets/API_images_guides_RFT_MultiReward_Chart.png)
...
![Reasoning Tokens Metric Graph](../../../assets/API_images_guides_RFT_ReasoningTokens_Chart.png)

If you are using [model graders](graders.md), you will likely want to monitor the token usage of these graders. Per-grader token usage statistics are available under the `event.data.usage.graders` object, and are broken down into:
...
![Model Grader Token Usage](../../../assets/API_images_guides_RFT_ModelGraderTokenUsage.png)
...
![Per Step Duration Graph](../../../assets/API_images_guides_RFT_PerStepDuration2.png)
...
Reinforcement fine-tuning jobs are directly integrated with our [evals product](evals.md). When you make a reinforcement fine-tuning job, a new eval is automatically created and associated with the job.

As validation steps are performed, the input prompts, model samples, grader outputs, and more metadata will be combined to make a new [eval run](evals.md) for that step. At the end of the job, you will have one run for each validation step. This allows you to compare the performance of the model at different steps, and to see how the model’s behavior has changed over the course of training.

You can find the eval associated with your fine-tuning job by viewing your job on the fine-tuning dashboard, or by finding the `eval_id` field on the [fine-tuning job object](../api-reference/fine-tuning/object.md).
...
You can pause a fine-tuning job at any time by using the [fine-tuning jobs API](../api-reference/fine-tuning/pause.md). Calling the pause API will tell the training process to create a new model snapshot, stop training, and put the job into a “Paused” state. The model snapshot will go through a normal safety screening process after which it will be available for you to use throughout the OpenAI platform as a normal fine-tuned model.

If you wish to continue the training process for a paused job, you can do so by using the [fine-tuning jobs API](../api-reference/fine-tuning/resume.md). This will resume the training process from the last checkpoint created when the job was paused and will continue training until the job is either completed or paused again.

### Grading with Tools

If you are training your model to [perform tool calls](function-calling.md), you will need to:

1. Provide the set of tools available for your model to call on each datapoint in the RFT training dataset. More info here in the [dataset API reference](../api-reference/fine-tuning/reinforcement-input.md).
2. Configure your grader to assign rewards based on the contents of the tool calls made by the model. Information on grading tools calls can be found [here in the grading docs](graders/index.md)
