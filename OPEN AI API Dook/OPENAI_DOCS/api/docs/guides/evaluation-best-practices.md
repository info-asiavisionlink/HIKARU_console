...
This guide provides high-level guidance on designing evals. To get started with the [Evals API](../api-reference/evals.md), see [evaluating model performance](evals.md).
...
Evals are structured tests for measuring a model’s performance. They help ensure accuracy, performance, and reliability, despite the nondeterministic nature of AI systems. They’re also one of the only ways to *improve* performance of an LLM-based application (through [fine-tuning](model-optimization.md)).
...
4. **Run and compare evals**  
   Use the [Evals API](evals.md) to create and run evals in the OpenAI dashboard.
...
Using models to judge output is cheaper to run and more scalable than human evaluation. Start with [`gpt-5.6`](../models/gpt-5.md) when you need a strong LLM judge, then validate agreement against your human labels before optimizing for cost or latency.
...
- **Recommendations**:
  ...
  - Use the most capable model to grade if you can. Start with [`gpt-5.6`](../models/gpt-5.md), then validate whether a specialized reasoning model performs better for your rubric or reference-answer set
...
Learn more about [reinforcement fine-tuning](reinforcement-fine-tuning.md) to create a data flywheel.
...
- [Evaluating model performance](evals.md)
- [How to evaluate a summarization task](/cookbook/examples/evaluation/how_to_eval_abstractive_summarization)
- [Fine-tuning](model-optimization.md)
- [Graders](graders.md)
- [Evals API reference](../api-reference/evals.md)