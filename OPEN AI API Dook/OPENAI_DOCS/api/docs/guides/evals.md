...
fetched_at: 2026-07-27T01:55:38Z
---
...
In this guide, we will focus on **configuring evals programmatically using the [Evals API](../api-reference/evals.md)**. If you prefer, you can also configure evals [in the OpenAI dashboard](https://platform.openai.com/evaluations).
...
[Datasets](evaluation-getting-started.md) instead.
...
This process is somewhat similar to behavior-driven development (BDD), where you begin by specifying how the system should behave before implementing and testing the system. Let’s see how we would complete each of the steps above using the [Evals API](../api-reference/evals.md).
...
To implement this use case, you can use either the [Chat Completions API](../api-reference/chat.md) or the [Responses API](../api-reference/responses.md). Both examples below combine a [developer message](text.md) with a user message containing the text of a support ticket.
...
Let’s set up an eval to test this behavior [via API](../api-reference/evals.md). An eval needs two key ingredients:

- `data_source_config`: A schema for the test data you will use along with the eval.
- `testing_criteria`: The [graders](graders.md) that determine if the model output is correct.
...
With our test data in place, let’s evaluate a prompt and see how it performs against our test criteria. Via API, we can do this by [creating an eval run](../api-reference/evals/createRun.md).
...
When we create the run, we set up a prompt using either a [Chat Completions](text.md) messages array or a [Responses](../api-reference/responses.md) input. This prompt is used to generate a model response for every line of test data in your data set. We can use the double curly brace syntax to template in the dynamic variable `item.ticket_text`, which is drawn from the current test data item.
...
To receive updates when a run succeeds, fails, or is canceled, create a webhook endpoint and subscribe to the `eval.run.succeeded`, `eval.run.failed`, and `eval.run.canceled` events. See the [webhooks guide](webhooks.md) for more details.

Depending on the size of your dataset, the eval run may take some time to complete. You can view current status in the dashboard, but you can also [fetch the current status of an eval run via API](../api-reference/evals/getRun.md):
