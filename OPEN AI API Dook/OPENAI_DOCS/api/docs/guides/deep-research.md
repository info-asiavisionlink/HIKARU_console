...
fetched_at: 2026-07-27T01:57:28Z
---
...
The [`o3-deep-research`](../models/o3-deep-research.md) and [`o4-mini-deep-research`](../models/o4-mini-deep-research.md) models can find, analyze, and synthesize hundreds of sources to create a comprehensive report at the level of a research analyst. These models are optimized for browsing and data analysis, and can use [web search](tools-web-search.md), [remote MCP](tools-remote-mcp.md) servers, and [file search](tools-file-search.md) over internal [vector stores](../api-reference/vector-stores.md) to generate detailed reports, ideal for use cases like:

- Legal or scientific research
- Market analysis
- Reporting on large bodies of internal company data

To use deep research, use the [Responses API](../api-reference/responses.md) with the model set to `o3-deep-research` or `o4-mini-deep-research`. You must include at least one data source: web search, remote MCP servers, or file search with vector stores. You can also include the [code interpreter](tools-code-interpreter.md) tool to allow the model to perform complex analysis by writing code.
...
Deep research models are agentic and conduct multi-step research. This means that they can take tens of minutes to complete tasks. To improve reliability, we recommend using [background mode](background.md), which allows you to execute long running tasks without worrying about timeouts or connectivity issues. In addition, you can also use [webhooks](webhooks.md) to receive a notification when a response is ready. Background mode can be used with the MCP tool or file search tool and is available for [Modified Abuse Monitoring](your-data.md) organizations.

While we strongly recommend using [background mode](background.md), if you choose to not use it then we recommend setting higher timeouts for requests. The OpenAI SDKs support setting timeouts e.g. in the [Python SDK](https://github.com/openai/openai-python?tab=readme-ov-file#timeouts) or [JavaScript SDK](https://github.com/openai/openai-node?tab=readme-ov-file#timeouts).
...
- Use [connectors](tools-remote-mcp.md) to pull in context from popular applications, like Dropbox and Gmail
...
For more details on the required schemas, how to build a compatible MCP server, and an example of a compatible MCP server, see our [deep research MCP guide](../mcp.md).
...
Read more about MCP risks & safety in our [MCP documentation](../mcp.md)
...
Learn more about deep research from these examples in the [OpenAI Cookbook](/cookbook).

- [Introduction to deep research](/cookbook/examples/deep_research_api/introduction_to_deep_research_api)
- [Deep research with the Agents SDK](/cookbook/examples/deep_research_api/introduction_to_deep_research_api_agents)
- [Building a deep research MCP server](/cookbook/examples/deep_research_api/how_to_build_a_deep_research_mcp_server/readme)