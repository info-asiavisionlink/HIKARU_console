...
fetched_at: 2026-07-27T01:57:54Z
---
...
If your MCP server is private, on-premises, or behind a firewall, use [Secure MCP Tunnel](secure-mcp-tunnels.md) to connect it to supported OpenAI products without exposing the server to the public internet. Download the latest public release from [openai/tunnel-client](https://github.com/openai/tunnel-client/releases/latest).

## Quickstart

Check out the examples below to see how remote MCP servers and connectors work through the [Responses API](../api-reference/responses/create.md). Both connectors and remote MCP servers can be used with the `mcp` built-in tool type.
...
The MCP tool (for both remote MCP servers and connectors) is available in the [Responses API](../api-reference/responses/create.md) in most recent models. Check MCP tool compatibility for your model [here](../models.md). When you’re using the MCP tool, you only pay for [tokens](../pricing.md) used when importing tool definitions or making tool calls. There are no additional fees involved per tool call.
...
each turn in a [conversation](conversation-state.md). We
...
If you are using [tool search](tools-tool-search.md), you can defer loading the functions exposed by an MCP server until the model decides it needs them. To do this, set `defer_loading: true` on the MCP server tool definition.
...
In other words, if you’re an organization with Data Residency in Europe, OpenAI will limit inference and storage of Customer Content to take place in Europe up until the point communication or data is sent to the MCP server. It is your responsibility to ensure that the MCP server also adheres to any Zero Data Retention or Data Residency requirements you may have. Learn more about Zero Data Retention and Data Residency [here](your-data.md).

## Usage notes

| API Availability | Rate limits | Notes |
| --- | --- | --- |
| [Responses](../api-reference/responses.md)  [Chat Completions](../api-reference/chat.md)  [Assistants](../api-reference/assistants.md) | **Tier 1**  200 RPM  **Tier 2 and 3**  1000 RPM  **Tier 4 and 5**  2000 RPM | [Pricing](../pricing.md)   [ZDR and data residency](your-data.md) |