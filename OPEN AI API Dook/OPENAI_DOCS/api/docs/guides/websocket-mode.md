...
fetched_at: 2026-07-27T01:55:15Z
---
...
The standalone [`/responses/compact` endpoint](../api-reference/responses/compact.md) returns a new compacted input window, not a response ID. After compaction, create a new response on your WebSocket connection using the compacted window as `input` (plus the next user/tool items).
...
- [Responses streaming events reference](../api-reference/responses-streaming.md)