...
2. If you are using [functions](function-calling.md), mark the ones you want to defer with `defer_loading: true`. If you are using [MCP servers](tools-connectors-mcp.md), set `defer_loading: true` on the MCP server tool definition.

### Use namespaces where possible

You can use tool search with deferred [functions](function-calling.md), [namespaces](function-calling.md), or [MCP servers](tools-connectors-mcp.md), but we recommend using namespaces or MCP servers when possible. Our models have primarily been trained to search those surfaces, and token savings are usually more material there.
...
Hosted tool search is the simplest path when you already know the full inventory of [functions](function-calling.md), [namespaces](function-calling.md), or [MCP servers](tools-connectors-mcp.md) you want the model to search. You declare them up front, add `{"type": "tool_search"}`, and let the API decide what to load.
...
- Use [function calling](function-calling.md) to define callable functions and custom tools.
- Use [Using tools](tools.md) for the broader tool landscape across Responses.