...
fetched_at: 2026-07-27T01:57:59Z
---
...
Check the [model page](../models.md) before enabling Programmatic Tool Calling.
...
Programmatic Tool Calling supports Zero Data Retention (ZDR) workflows without requiring a persistent code-execution container. ZDR must be enabled for the organization or project; setting `store: false` enables stateless continuation but does not enable ZDR by itself. Eligibility and retention depend on the complete request, including its model, tools, and third-party services; see [data controls](your-data.md).
...
For stateless reasoning-model requests, replay every returned reasoning item. Each item includes `encrypted_content` by default. See [conversation state](conversation-state.md) for the general stateless pattern.
...
- Use [function calling](function-calling.md) to define client-owned functions.
- Use [tool search](tools-tool-search.md) to defer large tool definitions until a model needs them.
- Use [conversation state](conversation-state.md) to continue stored or stateless Responses API requests.
- Review [data controls](your-data.md) before choosing a storage mode.