---
source_url: https://developers.openai.com/api/docs/guides/realtime-vad
...
It is enabled by default in [speech-to-speech](realtime-conversations.md) Realtime sessions, but is optional and can be turned off.
In [transcription](realtime-transcription.md) Realtime sessions, turn detection support depends on the transcription model. Models that support VAD default to `server_vad`, while `gpt-realtime-whisper` requires turn detection to be omitted or set to `null`.
...
You can configure VAD with the [`session.update`](../api-reference/realtime-client-events/session/update.md) client event by setting `session.audio.input.turn_detection`.
