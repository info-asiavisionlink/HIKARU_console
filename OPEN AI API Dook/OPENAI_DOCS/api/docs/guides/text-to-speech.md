...
fetched_at: 2026-07-27T01:58:12Z
---
...
The Audio API provides a [`speech`](../api-reference/audio/createSpeech.md) endpoint based on our [GPT-4o mini TTS (text-to-speech) model](../models/gpt-4o-mini-tts.md). It comes with 11 built-in voices and can be used to:
...
1. The [model](../api-reference/audio/createSpeech.md) you’re using
2. The [text](../api-reference/audio/createSpeech.md) to be turned into audio
3. The [voice](../api-reference/audio/createSpeech.md) that will speak the output
...
If you’re using the [Realtime API](realtime.md), note that the set of available voices is slightly different—see the [realtime conversations guide](realtime-conversations.md) for current realtime voices.
...
Custom voices enable you to create a unique voice for your agent or application. These voices can be used for audio output with the [Text to Speech API](../api-reference/audio/createSpeech.md), the [Realtime API](../api-reference/realtime.md), or the [Chat Completions API with audio output](audio.md).
...
Speech generation will work as usual. Simply specify the ID of the voice in the `voice` parameter when [creating speech](../api-reference/audio/createSpeech.md), or when initiating a [realtime session](../api-reference/realtime/create-call.md).
...
speech generation.](realtime.md)
[Audio and speech concepts

Review audio modalities, speech tasks, streaming, and request-based APIs.](audio.md)