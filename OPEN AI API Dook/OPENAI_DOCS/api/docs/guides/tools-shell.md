...
fetched_at: 2026-07-27T01:58:00Z
---
...
Build a coding agent with GPT-5.1 and the shell tool](/cookbook/examples/build_a_coding_agent_with_gpt-5.1)
...
Shell is available through the [Responses API](responses-vs-chat-completions.md). It’s not available via the Chat Completions API.
...
Shell tool with container\_auto
...
Use shell with container\_reference
...
Use the [Skills guide](tools-skills.md) for upload and versioning details.
...
For more details on data controls, see [ZDR and data residency](your-data.md).
...
Shell tool with domain\_secrets
...
For legacy migration details, see the older [Local shell guide](tools-local-shell.md).

## Use local shell with Agents SDK

If you are using the [Agents SDK](tools.md), you can pass your own shell executor implementation to the shell tool helper.
...
[OpenAI data controls](your-data.md) apply within OpenAI boundaries. However, data transmitted to third-party services over network connections is subject to their data retention policies. Ensure external endpoints meet your residency, retention, and compliance requirements.