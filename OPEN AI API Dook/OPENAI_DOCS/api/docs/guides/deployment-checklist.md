...
[Responses API](migrate-to-responses.md). It is OpenAI’s flagship
...
Choose a [GPT-5.6 model](latest-model.md) for the workload instead
...
[`reasoning.mode: "pro"`](reasoning.md) with
...
[tool search](tools-tool-search.md): add
...
[Programmatic Tool Calling](tools-programmatic-tool-calling.md)
...
[Multi-agent](responses-multi-agent.md) is a GPT-5.6 feature that
...
[Built-in tools](tools.md) are the API’s native capabilities.
...
[Compaction](compaction.md) is a context engineering tool: it
...
[Prompt caching](prompt-caching.md) automatically reduces latency
and cost when requests reuse the same long prefix. For high-volume workflows,
set
[`prompt_cache_key`](../api-reference/responses/create.md)
...
[Persisted reasoning](reasoning.md)
...
(ZDR)](your-data.md) requirements do not allow
...
Choose [`detail`](images-vision.md)
...
privacy-preserving
[`safety_identifier`](safety-best-practices.md)
...
Use [`background=True`](background.md) for requests that may take
...
[WebSocket mode](websocket-mode.md) is built for long-running,
