...
fetched_at: 2026-07-27T01:55:48Z
---
...
- We recommend starting with 50 well-crafted demonstrations and [evaluating the results](evals.md)
...
- Use historical data, expert data, logged data, or [other types of collected data](evals.md)

### Formatting your data

- Use [JSONL format](https://jsonlines.org/), with one complete JSON structure on every line of the training data file
- Use the [chat completions format](../api-reference/fine-tuning/chat-input.md)
...
- Capture results generated from your model using whatever technique is convenient - note that the [Responses API](../api-reference/responses.md) stores model responses for 30 days by default.
...
In addition to text completions, you can train the model to more effectively generate [structured JSON output](structured-outputs.md) or [function calls](function-calling.md).
...
With your test data uploaded, [create a fine-tuning job](../api-reference/fine-tuning/create.md) to customize a base model using the training data you provide. When creating a fine-tuning job, you must specify:

- A base model (`model`) to use for fine-tuning. This can be either an OpenAI model ID or the ID of a previously fine-tuned model. See which models support fine-tuning in the [model docs](../models.md).
...
Create a supervised fine-tuning job by calling the [fine-tuning API](../api-reference/fine-tuning.md):
...
The API responds with information about the fine-tuning job in progress. Depending on the size of your training data, the training process may take several minutes or hours. You can [poll the API](../api-reference/fine-tuning/retrieve.md) for updates on a specific job.
...
Note the `fine_tuned_model` property. This is the model ID to use in [Responses](../api-reference/responses.md) or [Chat Completions](../api-reference/chat.md) to make API requests using your fine-tuned model.
...
Fine-tune a reasoning model by grading its outputs.](reinforcement-fine-tuning.md)