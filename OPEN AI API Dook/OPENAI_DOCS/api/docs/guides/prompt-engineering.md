...
fetched_at: 2026-07-27T01:56:20Z
---
...
Here’s a simple example using the [Responses API](../api-reference/responses.md).
...
Here’s a simple example using the [Chat Completions API](../api-reference/chat.md).
...
A key choice to make when generating content through the API is which model you want to use - the `model` parameter of the code samples above. [You can find a full listing of available models here](../models.md). Here are a few factors to consider when choosing a model for text generation.

- **[Reasoning models](reasoning.md)** generate an internal chain of thought to analyze the input prompt, and excel at understanding complex tasks and multi-step planning. They are also generally slower and more expensive to use than GPT models.
...
When in doubt, [`gpt-5.6`](../models/gpt-5.md) offers a strong default for general-purpose text generation and prompt iteration.
...
- Pinning your production applications to specific [model snapshots](../models.md) (like `gpt-4.1-2025-04-14` for example) to ensure consistent behavior
...
- **Instructions:** Provide guidance to the model on how to generate the response you want. What rules should it follow? What should the model do, and what should the model never do? This section could contain many subsections as relevant for your use case, like how the model should [call custom functions](function-calling.md).
...
When constructing a message, you should try and keep content that you expect to use over and over in your API requests at the beginning of your prompt, **and** among the first API parameters you pass in the JSON request body to [Chat Completions](../api-reference/chat.md) or [Responses](../api-reference/responses.md). This enables you to maximize cost and latency savings from [prompt caching](prompt-caching.md).

## Few-shot learning

Few-shot learning lets you steer a large language model toward a new task by including a handful of input/output examples in the prompt, rather than [fine-tuning](model-optimization.md) the model. The model implicitly “picks up” the pattern from those examples and applies it to a prompt. When providing examples, try to show a diverse range of possible inputs with the desired outputs.
...
Models have different context window sizes from the low 100k range up to one million tokens for newer GPT-4.1 models. [Refer to the model docs](../models.md) for specific context window sizes per model.

## Prompting current GPT-5 series models

GPT models like [`gpt-5.6`](../models/gpt-5.md) benefit from precise instructions that explicitly provide the logic and data required to complete the task in the prompt. To get the most out of the latest GPT-5 series model, start with the current prompting guide.

[GPT-5.6 prompting guide

Get the most out of prompting the latest GPT-5 series model with current
guidance, practical examples, and migration notes.](latest-model.md)

### Prompting best practices for the latest GPT-5 series model

For the full current treatment, use the [latest GPT-5 prompting best practices](latest-model.md). The practical reminders below still apply.
...
For detailed guidance and prompt samples specific to coding, see the [latest GPT-5 prompting best practices](latest-model.md).
...
For detailed guidance and prompt samples specific to frontend development, see the [latest GPT-5 prompting best practices](latest-model.md).
...
For detailed guidance and prompt samples specific to building agents, see the [latest GPT-5 prompting best practices](latest-model.md).

## Prompting reasoning models

There are some differences to consider when prompting a [reasoning model](reasoning.md) versus prompting a GPT model. Generally speaking, reasoning models will provide better results on tasks with only high-level guidance. This differs from GPT models, which benefit from very precise instructions.
...
- [Prompting libraries & tools](/cookbook/related_resources#prompting-libraries--tools)
- [Prompting guides](/cookbook/related_resources#prompting-guides)
- [Video courses](/cookbook/related_resources#video-courses)
