...
fetched_at: 2026-07-27T01:55:11Z
---
...
The [Responses API](../api-reference/responses.md) is our new API primitive, an evolution of [Chat Completions](../api-reference/chat.md) which brings added simplicity and powerful agentic primitives to your integrations.
...
- Built-in tools like [web search](tools-web-search.md), [file search](tools-file-search.md), [computer use](tools-computer-use.md), [code interpreter](tools-code-interpreter.md), and [remote MCPs](tools-remote-mcp.md).
...
- [Reasoning](reasoning.md) models have a richer experience in the Responses API with [improved tool usage](reasoning.md). Starting with GPT-5.4, tool calling is not supported in Chat Completions with `reasoning: none`.
...
- The function-calling API shape is different, both for the function config on the request, and function calls sent back in the response. See the full difference in the [function calling guide](function-calling.md).
- The Responses SDK has an `output_text` helper, which the Chat Completions SDK does not have.
- In Chat Completions, conversation state must be managed manually. The Responses API has compatibility with the [Conversations API](conversation-state.md) for persistent conversations, or the ability to pass a `previous_response_id` to easily chain Responses together.
...
- Use the [Conversations API](conversation-state.md) when you need a persistent conversation object.
...
- Set `store: false` in the [store field](../api-reference/responses/create.md).
...
the [function calling docs](function-calling.md) for more detail on how function calling works in Responses.
...
Function-calling streams can also emit events such as `response.function_call_arguments.delta` and `response.function_call_arguments.done`. See the [streaming Responses guide](streaming-responses.md) and [Responses streaming events reference](../api-reference/responses-streaming.md).

### 8. Upgrade to native tools

If your application has use cases that would benefit from OpenAI’s native [tools](tools.md), you can update your tool calls to use OpenAI’s tools out of the box.
...
Based on developer feedback from the [Assistants API](../api-reference/assistants.md) beta, we’ve incorporated key improvements into the Responses API to make it more flexible, faster, and easier to use. The Responses API represents the future direction for building agents on OpenAI.

We now have Assistant-like and Thread-like objects in the Responses API. Learn more in the [migration guide](assistants/migration.md). As of August 26, 2025, we’re deprecating the Assistants API, with a sunset date of August 26, 2026.