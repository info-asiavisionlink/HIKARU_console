---
source_url: https://developers.openai.com/api/docs/guides/realtime-server-controls
fetched_at: 2026-07-27T01:58:19Z
---
...
1. When [establishing a peer connection](realtime-webrtc.md) you fetch and receive an SDP response from the Realtime API to configure the connection. If you used the sample code from the WebRTC guide, that looks something like this:
...
3. On a server, you can then [listen for events and configure the session](realtime-conversations.md) just as you would from a typical Realtime API WebSocket connection, using that call ID with the URL
