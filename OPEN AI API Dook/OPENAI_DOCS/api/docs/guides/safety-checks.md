---
source_url: https://developers.openai.com/api/docs/guides/safety-checks
fetched_at: 2026-07-27T01:58:41Z
---
...
With the introduction of [GPT-5](../models/gpt-5.md), we added some checks to find and halt hazardous information from being accessed. It’s likely some users will eventually try to use your application for things outside of OpenAI’s policies, especially in applications with a wide range of use cases.
...
- [Implement safety identifiers](safety-best-practices.md) for products where individual users interact with a model. Safety identifiers are recommended but not required.
...
The `safety_identifier` parameter is available in both the [Responses API](../api-reference/responses/create.md) and older [Chat Completions API](../api-reference/chat/create.md). The Realtime API supports the same concept through the `OpenAI-Safety-Identifier` header. To use safety identifiers, provide a stable ID for your end user on each request. Hash user email or internal user IDs to avoid passing any personal information.
...
- [Fine-tuning safety](supervised-fine-tuning.md)
- [Safety checks in computer use](tools-computer-use.md)