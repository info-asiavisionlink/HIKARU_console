---
source_url: https://developers.openai.com/api/docs/guides/realtime-mcp
fetched_at: 2026-07-27T01:58:18Z
---
...
You can attach tools to a Realtime session so the model can look up data, take actions, or call services during a live conversation. Tool configuration uses the same event surface whether your client is using a [WebRTC data channel](realtime-webrtc.md) or a [WebSocket](realtime-websocket.md).
...
- At the **session level** with `session.tools` in [`session.update`](../api-reference/realtime-client-events/session/update.md), if you want the tool available for the full session.
- At the **response level** with `response.tools` in [`response.create`](../api-reference/realtime-client-events/response/create.md), if you only need the tool for one turn.
...
For a full event-by-event walkthrough of function calling, see [Managing conversations](realtime-conversations.md).
...
3. While listing is still in progress, the model can’t call a tool that hasn’t loaded yet. If you want to wait before starting a turn that depends on those tools, listen for [`mcp_list_tools.completed`](../api-reference/realtime-server-events/mcp_list_tools/completed.md). The [`conversation.item.done`](../api-reference/realtime-server-events/conversation/item/done.md) event whose `item.type` is `mcp_list_tools` shows which tool names were actually imported. If import fails, you will receive [`mcp_list_tools.failed`](../api-reference/realtime-server-events/mcp_list_tools/failed.md).
...
7. Once the tool runs, you will see `response.mcp_call.in_progress`. On success, you will later receive a [`response.output_item.done`](../api-reference/realtime-server-events/response/output_item/done.md) event whose `item.type` is `mcp_call`; on failure, you will receive [`response.mcp_call.failed`](../api-reference/realtime-server-events/response/mcp_call/failed.md). The assistant message item and `response.done` complete the turn.
...
- [`mcp_list_tools.failed`](../api-reference/realtime-server-events/mcp_list_tools/failed.md): the Realtime API couldn’t import tools from the remote server or connector. Check `server_url` or `connector_id`, authentication, server connectivity, and any `allowed_tools` names you specified.
- [`response.mcp_call.failed`](../api-reference/realtime-server-events/response/mcp_call/failed.md): the model selected a tool, but the tool call didn’t complete. Inspect the event payload and the later `mcp_call` item for MCP protocol, execution, or transport errors.
...
If a tool requires approval, the Realtime API inserts an `mcp_approval_request` item into the conversation. **To continue**, send a new [`conversation.item.create`](../api-reference/realtime-client-events/conversation/item/create.md) event whose `item.type` is `mcp_approval_response`.
