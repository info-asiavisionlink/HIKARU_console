...
fetched_at: 2026-07-27T01:58:53Z
---
...
[Batch API](../api-reference/batch/create.md) queue limits are calculated based on the total number of input tokens queued for a given model. Tokens from pending batch jobs are counted against your queue limit. Once a batch job is completed, its tokens are no longer counted against that model’s limit.

Other important things worth noting:

- Rate limits are defined at the [organization level](production-best-practices.md) and at the project level, not user level.
- Rate limits vary by the [model](../models.md) being used.
...
- OpenAI sets an approved monthly usage limit for each organization. This is separate from the [spend limits](spend-limits.md) that you can configure for an organization or project.
...
| --- | --- | --- |
| Free | User must be in an [allowed geography](../supported-countries.md) | $100 / month |
...
To view a high-level summary of rate limits per model, visit the [models page](../models.md).
...
If your use case does not require immediate responses, you can use the [Batch API](batch.md) to more easily submit and execute large collections of requests without impacting your synchronous request rate limits.
...
Sending in a batch of prompts works exactly the same as a normal API call, except you pass in a list of strings to the prompt parameter instead of a single string. [Learn more in the Batch API guide](batch.md).