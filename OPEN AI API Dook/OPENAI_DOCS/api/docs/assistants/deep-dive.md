---
source_url: https://developers.openai.com/api/docs/assistants/deep-dive
fetched_at: 2026-07-27T01:55:56Z
---
...
Start from a template using the Assistants API with Next.js.](https://github.com/openai/openai-assistants-quickstart)
...
There are several concepts involved in building an app with the Assistants API, covered below in case it helps with your [migration to Responses](../guides/assistants/migration.md).

## Creating assistants

We recommend using OpenAI’s [latest models](../models.md) with
...
3. Use the `tool_resources` parameter to give the tools like `code_interpreter` and `file_search` access to files. Files are uploaded using the `File` [upload endpoint](../api-reference/files/create.md) and must have the `purpose` set to `assistants` to be used with this API.
...
You can attach a maximum of 20 files to `code_interpreter` and 10,000 files to `file_search` (using `vector_store` [objects](../api-reference/vector-stores/object.md)). For vector stores created starting in November 2025, the `file_search` limit is 100,000,000 files.
...
When using the File Search tool, we recommend setting the max\_prompt\_tokens to
...
the max\_prompt\_tokens limits altogether to get the highest quality results.
...
Messages created by Assistants may contain [`annotations`](../api-reference/messages/object.md) within the `content` array of the object. Annotations provide information around how you should annotate the text in the Message.

There are two types of Annotations:

1. `file_citation`: File citations are created by the [`file_search`](tools/file-search.md) tool and define references to a specific file that was uploaded and used by the Assistant to generate the response.
2. `file_path`: File path annotations are created by the [`code_interpreter`](tools/code-interpreter.md) tool and contain references to the files generated by the tool.
...
Note: `tool_resources` associated with the Assistant cannot be overridden during Run creation. You must use the [modify Assistant](../api-reference/assistants/modifyAssistant.md) endpoint to do this.

#### Run lifecycle

Run objects can have multiple statuses.

![Run lifecycle - diagram showing possible status transitions](../../../assets/API_docs_images_diagram-run-statuses-v2.png)

| Status | Definition |
| --- | --- |
| `queued` | When Runs are first created or when you complete the `required_action`, they are moved to a queued status. They should almost immediately move to `in_progress`. |
| `in_progress` | While in\_progress, the Assistant uses the model and tools to perform steps. You can view progress being made by the Run by examining the [Run Steps](../api-reference/runs/step-object.md). |
| `completed` | The Run successfully completed! You can now view all Messages the Assistant added to the Thread, and all the steps the Run took. You can also continue the conversation by adding more user Messages to the Thread and creating another Run. |
| `requires_action` | When using the [Function calling](tools/function-calling.md) tool, the Run will move to a `required_action` state once the model determines the names and arguments of the functions to be called. You must then run those functions and [submit the outputs](../api-reference/runs/submitToolOutputs.md) before the run proceeds. If the outputs are not provided before the `expires_at` timestamp passes (roughly 10 mins past creation), the run will move to an expired status. |
| `expired` | This happens when the function calling outputs were not submitted before `expires_at` and the run expires. Additionally, if the runs take too long to execute and go beyond the time stated in `expires_at`, our systems will expire the run. |
| `cancelling` | You can attempt to cancel an `in_progress` run using the [Cancel Run](../api-reference/runs/cancelRun.md) endpoint. Once the attempt to cancel succeeds, status of the Run moves to `cancelled`. Cancellation is attempted but not guaranteed. |
...
If you are not using [streaming](overview.md), in order to keep the status of your run up to date, you will have to periodically [retrieve the Run](../api-reference/runs/getRun.md) object. You can check the status of the run each time you retrieve the object to determine what your application should do next.
...
![Run steps lifecycle - diagram showing possible status transitions](../../../assets/API_docs_images_diagram-2.png)
...
2. `tool_calls`: This Run Step is created when the Assistant calls a tool. Details around this are covered in the relevant sections of the [Tools](tools.md) guide.
