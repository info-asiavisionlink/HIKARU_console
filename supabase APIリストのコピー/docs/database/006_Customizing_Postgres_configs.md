---
タイトル: Customizing Postgres configs
URL: https://supabase.com/docs/guides/database/custom-postgres-config
カテゴリ: database
更新日: 2026-08-02
タグ: configs, custom-postgres-config, customizing, database, postgres
---
...
Configuring Postgres for your Supabase project.
...
`log_duration`| Logs how long a query ran for, but not the entire query itself.  
...
`log_min_duration_statement`| Logs the duration and text of completed statements whose execution time meets or exceeds the specified threshold  
`log_min_error_statement`| Determines if the query that induced a log should be recorded as a subfield based on the event's [severity level](<https://www.postgresql.org/docs/current/runtime-config-logging.html#RUNTIME-CONFIG-SEVERITY-LEVELS>)  
...
---|---|---|---  
[cron.log_statement](<https://github.com/citusdata/pg_cron#extension-settings>)| CLI only| Yes| `--config cron.log_statement=false`  
...
[log_autovacuum_min_duration](<https://www.postgresql.org/docs/current/runtime-config-logging.html#GUC-LOG-AUTOVACUUM-MIN-DURATION>)| CLI only| No| `--config log_autovacuum_min_duration=10s`  
[log_checkpoints](<https://www.postgresql.org/docs/current/runtime-config-logging.html#GUC-LOG-CHECKPOINTS>)| CLI only| No| `--config log_checkpoints=true`  
[log_connections](<https://www.postgresql.org/docs/current/runtime-config-logging.html#GUC-LOG-CONNECTIONS>)| CLI only| No| `--config log_connections=true`  
[log_disconnections](<https://www.postgresql.org/docs/current/runtime-config-logging.html#GUC-LOG-DISCONNECTIONS>)| CLI only| No| `--config log_disconnections=true`  
[log_lock_waits](<https://www.postgresql.org/docs/current/runtime-config-logging.html#GUC-LOG-LOCK-WAITS>)| CLI + SQL| No| `--config log_lock_waits=true`  
[log_recovery_conflict_waits](<https://www.postgresql.org/docs/current/runtime-config-logging.html#GUC-LOG-RECOVERY-CONFLICT-WAITS>)| CLI only| No| `--config log_recovery_conflict_waits=true`  
[log_replication_commands](<https://www.postgresql.org/docs/current/runtime-config-logging.html#GUC-LOG-REPLICATION-COMMANDS>)| CLI + SQL| No| `--config log_replication_commands=true`  
[log_startup_progress_interval](<https://www.postgresql.org/docs/current/runtime-config-logging.html#GUC-LOG-STARTUP-PROGRESS-INTERVAL>)| CLI only| No| `--config log_startup_progress_interval=10s`  
[log_temp_files](<https://www.postgresql.org/docs/current/runtime-config-logging.html#GUC-LOG-TEMP-FILES>)| CLI + SQL| No| `--config log_temp_files=10MB`  
...
To start:
  ...
  1. [Install](</docs/guides/local-development>) Supabase CLI 1.69.0+.
