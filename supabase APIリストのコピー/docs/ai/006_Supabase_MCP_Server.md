---
タイトル: Supabase MCP Server
URL: https://supabase.com/docs/guides/ai-tools/mcp
カテゴリ: ai
更新日: 2026-08-02
タグ: ai, ai-tools, mcp, server, supabase
---
...
Connecting an LLM to your Supabase projects carries security risks. Read our security best practices before running the MCP server.
...
Need help?[View Claude Code docs](<https://code.claude.com/docs/en/mcp>)
...
When using [Supabase CLI](</docs/guides/local-development>) for local development, the MCP server is available at `http://localhost:54321/mcp`.
...
We recommend the following best practices to mitigate security risks when using the Supabase MCP server:
  ...
  * **Feature groups** : Restrict which tool groups are available using the `features` configuration option. This helps reduce the attack surface and limits the actions that LLMs can perform to only those that you need.
