---
タイトル: Debugging guide
URL: https://supabase.com/docs/guides/monitoring-and-debugging/debugging
カテゴリ: platform
更新日: 2026-08-02
タグ: debugging, guide, monitoring-and-debugging, platform
---
...
- [Follow these debugging steps#](#follow-these-debugging-steps)
- [Check the request stack#](#check-the-request-stack)
- [Read the logs#](#read-the-logs)
- [Find the guide for your symptom#](#find-the-guide-for-your-symptom)
- [Per-product debugging#](#per-product-debugging)

## 概要

Isolate and fix Supabase issues by reading the error, isolating the failing layer, and gathering evidence from logs.
...
Work through these steps in order, skipping straight to a fix before you have evidence for the cause is the most common way to waste time on a bug.
  ...
  3. **Gather evidence for that layer.** Query its logs, run the security and performance advisors, and inspect the schema. Logs are the primary tool, and the layer you identified in the previous step tells you which log source to query. See Read the logs below.
