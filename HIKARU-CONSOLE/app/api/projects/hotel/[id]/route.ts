...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (assignments !== undefined) {
    ...
    const normalizedAfter = (assignments as any[]).map((a: any) => ({
      assignee_type: a.assignee_type as 'employee' | 'partner',
      assignee_id:   a.assignee_id as string,
    }))

    const removed = diffRemoved(before, normalizedAfter)
    if (removed.length > 0) {
    ...
    if (assignments.length) {
      ...
      if (aErr) {
        ...
        const newlyAdded = diffAssignments(before, normalizedAfter)
