...
type PartnerRow = { auth_user_id: string | null; company_id: string }
type ProfileLine = { line_user_id: string | null; line_notify_enabled: boolean }

// PATCH /api/partners/[id]/line - 協力業者 LINE連携情報を更新
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!partner) return NextResponse.json({ error: '協力業者が見つかりません' }, { status: 404 })
  if (!partner.auth_user_id) {
    return NextResponse.json({ error: 'この協力業者にはログインアカウントが紐付いていません' }, { status: 400 })
  }
...
// GET /api/partners/[id]/line - LINE連携状態を取得
